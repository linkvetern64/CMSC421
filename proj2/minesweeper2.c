 ...
 * handles user input and prints the game status. Now featuring /dev/ms_stats
 * which will handle stats of game played
 */
/*
...
/* temporary buffer to hold stats data */
...
static void clear_revealed(int pos, int x, int y)
{
	...
	if (sym == '?' || sym == '-' || (sym - '0') >= 1 || (sym - '0') <= 9) {
		user_view[pos] = '.';
	}

	if (y + 1 <= 9 && y + 1 >= 0) {
		pos_equals_mark(10 * (y + 1) + x);
	}
	if (y - 1 <= 9 && y - 1 >= 0) {
		pos_equals_mark(10 * (y - 1) + x);
	}
	if (x - 1 <= 9 && x - 1 >= 0) {
		pos_equals_mark(10 * (y + 0) + (x - 1));
	}
	if (x - 1 <= 9 && x - 1 >= 0 && y + 1 <= 9 && y + 1 >= 0) {
		pos_equals_mark(10 * (y + 1) + (x - 1));
	}
	if (x - 1 <= 9 && x - 1 >= 0 && y - 1 <= 9 && y - 1 >= 0) {
		pos_equals_mark(10 * (y - 1) + (x - 1));
	}
...
static ssize_t delta_mines(int ops, int x, int y, size_t count)
{
	...
	if (!game_over) {
		...
		if (!check_won()) {
			scnprintf(game_status, 80, "%d Marked of %d",
				  mines_marked, NUM_MINES);
		}
...
static ssize_t update_board(const char *ubuf, size_t count)
{
	...
	//Check against 8 bytes or lower.
	comp = 8;
	count = min(count, comp);

	//net_sig is set in cs421_bottom
	if (net_sig) {
		for (l = 0; l <= count; l++) {
			buf[l] = ubuf[l];
		}
	...
	case 'r':
		...
		scnprintf(game_status, 80, "%d Marked of %d", mines_marked,
			  NUM_MINES);
	...
	case 'a':
		//only accessible if user is local root
...
static irqreturn_t cs421net_bottom(int irq, void *cookie)
{
	...
	counter = 0;
	packet = cs421net_get_data(&packet_length);

	//parse through packet for \n
	if (packet != NULL && packet_length > 0) {
		for (i = 0; i < (size_t) packet_length; i++) {
			if ((int)packet[(int)i] != 10
			    && (int)packet[(int)i] != 13) {
...
static int __init minesweeper_init(void)
{
	...
	if (request_threaded_irq
	    (CS421NET_IRQ, cs421net_top, cs421net_bottom, 0, "421 IRQ",
	     NULL) == 0) {
