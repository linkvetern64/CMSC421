 ...
 * null-terminated.
 *
 * @len: pointer to where to write the data length
 * Return: data buffer, or %NULL if none pending
 */
const char *cs421net_get_data(size_t * const len)
{
	struct incoming_data *oldest;
	unsigned long flags;

	spin_lock_irqsave(&lock, flags);
	oldest = list_first_entry_or_null(&cs421net_list, struct incoming_data, list);
	if (!oldest) {
...
static void cs421net_work_func(struct work_struct *work)
{
	...
	if (trigger_irq(CS421NET_IRQ) < 0) {
		...
		wait_for_completion_interruptible(&retrieved);
